...
/**
 \file  tbb_main.c
 \brief Threading Building Blocks implementation of semi-infinite diffusion equation
*/
...
int main(int argc, char* argv[])
{
	...
	for (step = 1; step < steps + 1; step++) {
		...
		if (step % checks == 0) {
			...
			start_time = GetTimer();
			check_solution_lambda(conc_old, conc_lap, nx, ny, dx, dy, nm, elapsed, D, &rss);
